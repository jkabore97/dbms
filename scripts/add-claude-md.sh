#!/usr/bin/env bash
# Adds CLAUDE.md — persistent project rules Claude Code reads automatically
# at the start of every session, regardless of how that session was started.
set -euo pipefail
REPO_URL="https://github.com/jkabore97/dbms.git"
WORK_DIR="${TMPDIR:-/tmp}/kaj-claudemd-$$"
echo "==> Cloning $REPO_URL"
git clone "$REPO_URL" "$WORK_DIR"
cd "$WORK_DIR"

echo "==> Writing CLAUDE.md"
cat > "CLAUDE.md" <<'KAJ_EOF_0'
...
git add -A
git commit -m "Add CLAUDE.md: persistent project rules from lessons learned"
